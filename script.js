$(document).ready(function() {
    let lighted = localStorage.getItem("lighting");
    const todoForm = $("form");
    ...
    if (lighted == "light" || lighted === null) {
        $("body").addClass("lightmode");
        $("#todo-input, #todo-date, #todo-time, #new-input, #new-date, #new-time").css("color-scheme", "light");
    ...
    $("#lighting").click(function() {
        if (lighted == "light") {
            ...
            $("#todo-input, #todo-date, #todo-time, #new-input, #new-date, #new-time").css("color-scheme", "dark");
        ...
        lighted = localStorage.getItem("lighting");
    });
    ...
    function createTodoItem(todo, todoIndex) {
        ...
        const editButton = todoLI.find(".edit-button");
    ...
    $("#submit-edit").click(function(event) {
        ...
        const text = $("#new-input").val().trim();
        const time = $("#new-time").val();
        const date = $("#new-date").val();
        const complete = $("#tada").is(":checked");
        if (!text) {
            return;
        } 
        
        editTodoItem(editing, text, time, date, complete);
        $modal.hide();
    });

    function editTodoItem(todoIndex, ntext, ntime, ndate, ncompleted) {
        allTodos[todoIndex] = {
            text: ntext,
            time: ntime,
            date: ndate,
